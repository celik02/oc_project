...
class FrenetMPCController:
    ...
    def run_step(self, ego_vehicle, preceding_vehicle, target_speed, next_waypoint):
        ...
        # In run_step method, after calculating ego_frenet and lead_frenet
        ...
        # Calculate relative distance in Frenet frame
        ego_location = ego_vehicle.actor.get_location()
        ...
        # Debug prints
        # print(f"Ego d: {d0:.2f}, Lead d: {lead_d:.2f}, Relative s: {relative_s:.2f}")

        # Calculate d   esired lateral position based on target lane
        ...
        # visualize the vehicle trajectory following the optimal control by using the frenet_vehicle_dynamics_casadi function
        # Draw the predicted trajectory in CARLA
        ...
        # draw the predicted trajectory in CARLA using the frenet coordinates s and d relative to the current position of the car
        # Continue the code to draw the predicted trajectory if we were to take all of these consecutive actions
        # Visualize the predicted trajectory in CARLA
        if self.carla_manager is not None:
            # Get the ego vehicle's current location and waypoint directions
            ego_location = ego_vehicle.actor.get_location()
            forward_vec = waypoint.transform.get_forward_vector()
            right_vec = waypoint.transform.get_right_vector()
            ...
            for i in range(len(predicted_trajectory)):
                # Get state values relative to the initial state
                ds = predicted_trajectory[i][0] - predicted_trajectory[0][0]  # Change in s (longitudinal progress)
                dd = predicted_trajectory[i][1] - predicted_trajectory[0][1]  # Change in d (lateral offset)
                
                # Calculate world position by projecting in forward and right directions
                # This transformation converts from Frenet coordinates (s,d) to world coordinates (x,y)
                world_pos = ego_location + carla.Location(
                    forward_vec.x * ds + right_vec.x * dd,
                    forward_vec.y * ds + right_vec.y * dd,
                    z_offset
                )
